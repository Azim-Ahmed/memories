...
export const deletePost = async (req, res) => {
    const { id } = req.params
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send('No post for this Id')
...
export const likePost = async (req, res) => {
    ...
    const updatedLikePost = await PostMessage.findByIdAndUpdate(id, { likeCount: postData.likeCount + 1 }, { new: true })
